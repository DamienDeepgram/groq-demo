...
async function promptAI(socketId, msg) {
    ...
    // Make sure to configure your GROQ API Key in config.json for this to work
    if(data && !data.err){
      lineIndex++;
      let reply = data.response.content;
      updateAudio(reply);
      addText(reply, true);
    } else {
      console.log(data.err);
      alert('Error: You must configure your GROQ API Key in the config.json to use the "Respond with AI" feature.');
    }
}
