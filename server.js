...
const GROQ_AUTH_API = 'https://api.groq.com/v1/auth/get_token';
...
async function promptAI(socketId, model, message){
  ...
  const response = await fetch(
    GROQ_API, 
    {
      ...
      body: JSON.stringify(groq_api_request)
    })
...
app.get("/chat", async (req, res) => {
  ...
  if(!process.env.GROQ_API_KEY){
    res.status(500).send({ err: 'No Groq API Key set in the .env file' });
    return;
  }
  ...
  if(!socket){
    ...
    socket.on("connection", (clientSocket) => {
      ...
      if(process.env.GROQ_API_KEY){
        groqChats[socketId] = { 
          history: [],
          system_prompt: 'You are a helpful assistant. You respond with short responses like you are speaking verbally with the user.'
        };
      }
